...
function TicTacToeGame() {
  ...
  useWebMCP({
    ...
    handler: async ({ position, player }) => {
      // Update game state
      const newState = makeMove(gameState, position, player);
...
```typescript
// MCP server tool that returns an iframe
server.registerTool({
  name: "showTicTacToeGame",
  description: "Display an interactive TicTacToe game",
      ...
      toolResult: {
        content: [{
          ...
          resource: {
            ...
            metadata: {
              uiType: "externalUrl",
              url: "https://example.com/games/tictactoe"
            }
          }
        }]
      }
...
sequenceDiagram
    ...
    I->>C: postMessage: "mcp-init"
    C-->>I: postMessage: "mcp-init-ack"
  ...
  <Accordion title="remoteDom - Dynamic Content">
    ...
    ```javascript
    {
      ...
      resource: {
        ...
        metadata: {
          uiType: "remoteDom",
          initialHtml: "<div id='root'></div>",
          updateChannel: "wss://example.com/updates"
        }
      }
    }
    ```
...
The MCP-B packages (`@mcp-b/react-webmcp`, `@mcp-b/transports`, `@mcp-b/core`) enable WebMCP functionality today by:
...
- `@mcp-b/core` - Core WebMCP polyfill
