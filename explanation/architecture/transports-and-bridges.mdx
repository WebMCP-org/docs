...
The parent uses an `IframeParentTransport`. The child uses an `IframeChildTransport`. The [`@mcp-b/mcp-iframe`](/packages/mcp-iframe/overview) package wraps that setup in a custom element and namespaces the forwarded tools to prevent collisions.
...
For API details, see [@mcp-b/transports](/packages/transports/overview), [@mcp-b/mcp-iframe](/packages/mcp-iframe/overview), [@mcp-b/webmcp-local-relay](/packages/webmcp-local-relay/overview), and [@mcp-b/chrome-devtools-mcp](/packages/chrome-devtools-mcp/overview).
