...
When [`@mcp-b/global`](/packages/global/reference) initializes, it installs a `BrowserMcpServer` that keeps the core behavior and adds a separate extension layer:
...
The [`@mcp-b/webmcp-types`](/packages/webmcp-types/reference) package defines two important interfaces:
...
**If you are building a library or adapter**, stay on the core side. Use [`@mcp-b/webmcp-types`](/packages/webmcp-types/reference) and [`@mcp-b/webmcp-polyfill`](/packages/webmcp-polyfill/reference).
