...
For read and test operations, Chromium currently exposes a separate [navigator.modelContextTesting](/explanation/webmcp/standard-api) interface. For the ongoing spec questions around that split, see [Spec Status and Limitations](/explanation/design/spec-status-and-limitations).
...
When [`@mcp-b/global`](/packages/global/overview) initializes, it installs a `BrowserMcpServer` that keeps the core behavior and adds a separate extension layer:
...
The [`@mcp-b/webmcp-types`](/packages/webmcp-types/overview) package defines two important interfaces:
...
**If you are building a library or adapter**, stay on the core side. Use [`@mcp-b/webmcp-types`](/packages/webmcp-types/overview) and [`@mcp-b/webmcp-polyfill`](/packages/webmcp-polyfill/overview).

**If you are building an application**, start by asking whether you need only site-exposed tools. If yes, stay on the strict path with [`@mcp-b/webmcp-polyfill`](/packages/webmcp-polyfill/overview). If you also need prompts, resources, transport, or desktop-agent connectivity, move up to [`@mcp-b/global`](/packages/global/overview) and the [`@mcp-b/webmcp-ts-sdk` reference](/packages/webmcp-ts-sdk/overview).
