...
The `@mcp-b/global` polyfill must be imported at the **top level** of your application, **before** any tools are registered.

```tsx "src/app/layout.tsx"
'use client'  // Required - layout must be client component
...
```
src/app/layout.tsx          → Has <EmbeddedAgent />
src/app/(calendar)/layout.tsx → Also has <EmbeddedAgent />
...
<Tabs>
  <Tab title="Root layout">
    Use when the agent doesn't need feature-specific context:
    ```tsx "src/app/layout.tsx"
    'use client';
  ...
  <Tab title="Feature layout">
    Use when the agent needs specific context:
    ...
    export default function CalendarLayout({ children }) {
      return (
        <CalendarProvider>
          <CalendarTools />      {/* Tools need context */}
          <MyEmbeddedAgent />    {/* Agent here, NOT in root */}
          {children}
        </CalendarProvider>
      );
    }
...
- [ ] Root layout has `'use client'` directive
- [ ] `@mcp-b/global` is imported at the top of root layout
...
src/
├── app/
│   ├── layout.tsx          # 'use client', imports @mcp-b/global
│   └── (feature)/
│       ├── layout.tsx      # Feature provider + tools + embedded agent
...
1. Polyfill loads first (root layout)
2. Tools have access to context (feature layout)
3. Agent renders once (feature layout only)
4. Pages can still use Server Components for data fetching
