...
<Steps>
  ...
  <Step title="Handle cleanup">
    Call `unregister()` when components unmount (see [Tool Lifecycle](/concepts/tool-registration#tool-lifecycle))
  </Step>
</Steps>

<Info>
  **Tool Discovery**: AI agents discover and call registered tools via [TabClientTransport](/packages/transports#tab-transport-examples) using `client.listTools()`. Your components just register and unregister tools—the transport handles discovery.
</Info>
...
Always unregister tools when components are destroyed. See [Tool Lifecycle](/concepts/tool-registration#tool-lifecycle) for details.
...
<CardGroup cols={2}>
  ...
  <Card title="TabClientTransport" icon="plug" href="/packages/transports#tab-transport-examples">
