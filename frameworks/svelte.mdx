...
<Steps>
  ...
  <Step title="Have Svelte configured">
    This guide covers Svelte 4/5 and SvelteKit
  </Step>
</Steps>

<Info>
  For general SSR patterns, see [Framework Gotchas](/ai-frameworks/framework-gotchas). For schema validation, see [Schemas Guide](/concepts/schemas).
</Info>
...
<Info>
  **Tool Discovery**: AI agents discover registered tools via [TabClientTransport](/packages/transports#tab-transport-examples) using `client.listTools()`. Your components just register and unregister tools—no need to track them internally.
</Info>
