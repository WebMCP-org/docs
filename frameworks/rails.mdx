...
<Info>
  For response formatting and schema validation, see [Tool Registration](/concepts/tool-registration) and [Schemas Guide](/concepts/schemas).
</Info>

<Info>
  **Tool Discovery**: AI agents discover registered tools via [TabClientTransport](/packages/transports#tab-transport-examples) using `client.listTools()`. Your controllers just register and unregister tools—no need to track them internally.
</Info>
