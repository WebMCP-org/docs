...
The first script tag loads `@mcp-b/global`, which sets up `navigator.modelContext`. The tool registration is the same pattern from the [first tool tutorial](/tutorials/first-tool). The last script tag loads `embed.js`, which creates a hidden iframe that connects the page's tools to the local relay via WebSocket.
...
Open any website that uses WebMCP (for example, [webmcp.sh](https://webmcp.sh)) in a separate tab. Then ask the agent to list sources again. You will see tools from multiple tabs. The relay aggregates tools from all connected pages.
...
- See the [`webmcp-local-relay` reference](/reference/tools/webmcp-local-relay) for CLI options, security settings, and architecture details
