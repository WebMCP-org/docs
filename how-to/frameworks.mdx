...
<Tabs>
  ...
  <Tab title="@mcp-b/webmcp-polyfill (strict core)">
    Strict-core only: installs `navigator.modelContext` with `registerTool` and `unregisterTool`. No MCP extensions, no transports.
...
<Tabs>
  <Tab title="React">
    ...
    | Package | Use when |
    |---------|----------|
    | [**@mcp-b/react-webmcp**](/packages/react-webmcp/overview) | You want the full MCP-B surface: Zod schemas, prompts, resources, sampling, elicitation |
    | [**usewebmcp**](/packages/usewebmcp/overview) | You want strict-core `navigator.modelContext` tools only |
