...
description: "Reference for the strict WebMCP core polyfill that installs navigator.modelContext."
keywords: ["@mcp-b/webmcp-polyfill", "polyfill", "navigator.modelContext", "initializeWebMCPPolyfill", "registerTool", "modelContextTesting", "strict core"]
icon: "puzzle-piece"
---
...
<Note>
The polyfill does not provide `listTools()` or `callTool()` on `navigator.modelContext`. For tool listing and execution, use `navigator.modelContextTesting` (when the testing shim is enabled) or switch to [`@mcp-b/global`](/packages/global/reference).
</Note>
