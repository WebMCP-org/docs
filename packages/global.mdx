...
<Accordion title="Implementation Detail: Two-Bucket Tool Management">
  WebMCP uses a two-bucket system: base tools (via `provideContext()`) and dynamic tools (via `registerTool()`). Dynamic tools persist independently, making them perfect for component lifecycle management. See [Core Concepts](/concepts#tool-registration) for details.
</Accordion>
