...
<Info>
**New to MCP-UI?** This guide walks you through building complete bidirectional applications where AI agents can both display interfaces and interact with them through dynamically registered tools.
</Info>
...
<Tip>
**The magic:** Your embedded app can register tools that the AI can call, creating true bidirectional interaction where the UI is also an API.
</Tip>
...
<Info>
**Understanding the flow:** The following diagram shows how AI agents, MCP servers, host applications, and embedded apps work together to enable bidirectional tool interaction.
</Info>
...
<Note>
Your MCP server (`worker/mcpServer.ts`) provides the **entry point** for AI agents. It exposes tools that return UI resources, which then register their own tools back to the AI.
</Note>
...
- The MCP server returns a `UIResource` with `type: 'externalUrl'`
- The `iframeUrl` points to your embedded app
- The host application (Claude Desktop, chat-ui) renders this as an iframe
...
<Tip>
**No build step required!** The Vanilla template uses `@mcp-b/global` via CDN to register tools. Your app runs inside an iframe and communicates with the parent via `postMessage`.
</Tip>
...
- `@mcp-b/global` IIFE provides `window.navigator.modelContext` globally
- Call `provideContext({ tools: [...] })` to register tools with the parent
- **Tool handlers and UI buttons share the same state management logic**
- Parent-child ready protocol ensures reliable bidirectional communication
</Note>

## Part 3: The Embedded App (React)

<Info>
**Modern React approach:** Use `initializeWebModelContext()` before React renders, then register tools with the `useWebMCP` hook. Zod schemas provide type-safe validation.
</Info>

### Example: React Template

<Warning>
**CRITICAL:** Call `initializeWebModelContext()` **BEFORE** React renders. This must happen at module scope in your entry point.
</Warning>
...
- `initializeWebModelContext()` **must run BEFORE React renders** (critical timing!)
- `useWebMCP` hook automatically registers tools on mount and unregisters on unmount
- Zod schemas provide **type-safe validation** with excellent DX
- **Single source of truth:** Tool handlers and UI share the same React state
</Note>
...
<Steps>
  ...
  <Step title="Test the Interaction">
    1. AI calls `showTemplateApp` → iframe appears in side panel
    2. AI calls `template_get_message` → reads current state from iframe
    3. AI calls `template_update_message` → updates the UI dynamically
    4. You can also click UI buttons - they use the same logic!

    <Tip>
    The bidirectional flow means both AI and humans can interact with your app seamlessly.
    </Tip>
  </Step>
</Steps>
...
<Info>
Following these patterns ensures your app is maintainable, secure, and provides a great experience for both AI and human users.
</Info>

### 1. Share Logic Between UI and Tools

<Tip>
**Single source of truth:** Both your UI handlers and WebMCP tools should call the same underlying functions. This ensures consistency and reduces bugs.
</Tip>
...
<Note>
Deploy your MCP-UI app to Cloudflare Workers for production use. The build process creates optimized bundles for both your MCP server and embedded app.
</Note>
...
<Steps>
  ...
  <Step title="Deploy to Cloudflare Workers">
    ...
    <Tip>
    Your Worker URL becomes the iframe URL that MCP servers return in UI resources.
    </Tip>
  </Step>
