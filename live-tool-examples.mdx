...
<Note>
  These examples require the **MCP-B browser extension** to enable AI agent integration. The tools will still work manually without the extension.
</Note>

## Prerequisites

To use these tools with AI agents:
...
3. Try the example prompts provided with each tool
  ...
  useEffect(() => {
    ...
    return () => {
      if (window.navigator?.modelContext?.unregisterTool) {
        window.navigator.modelContext.unregisterTool('my_tool');
      }
    };
  }, []);
