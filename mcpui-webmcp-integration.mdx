...
<Note>
**Novel Integration Pattern**: MCP-UI serves HTML/iframes that contain MCP servers. The parent page connects using **IframeParentTransport** or **TabClientTransport**, and it **automatically discovers** all tools inside the iframe - no manual bridging needed!
</Note>
...
<CardGroup cols={3}>
  <Card title="Live Demos" icon="globe" href="#try-it-live">
    Try the TicTacToe game demo
  </Card>
  <Card title="Simple Idea" icon="lightbulb" href="#the-simple-idea">
    Understand the core concept
  </Card>
  <Card title="Quick Start" icon="bolt" href="#quick-start-with-create-webmcp-app">
    Get started with CLI
  </Card>
  <Card title="Real Example" icon="gamepad" href="#real-world-example-tictactoe-game">
    TicTacToe implementation
  </Card>
  <Card title="Deployment" icon="rocket" href="#deployment-architecture">
    Production setup
  </Card>
...
<Info>
This complete production implementation from the [mcp-ui-webmcp repository](https://github.com/WebMCP-org/mcp-ui-webmcp) demonstrates all integration patterns in action.
</Info>

### System Architecture

<Note>
The following diagram shows how all three components (Chat UI, MCP Server, and Embedded App) work together to enable AI gameplay.
</Note>
...
<AccordionGroup>
  <Accordion title="Shared Logic Pattern" icon="code-merge">
    The TicTacToe app demonstrates best practices where UI handlers and WebMCP tools call the same underlying functions (`game.makeMove()`, `game.reset()`).
    ...
    **Benefits:**
    ...
    <Tip>
    Always share your state management logic between UI and tools!
    </Tip>
  </Accordion>

  <Accordion title="Tool Annotations" icon="tags">
    Provide metadata for better AI interaction:
    ...
    These hints help AI agents make better decisions about when to call tools.
  </Accordion>

  <Accordion title="Parent-Ready Protocol" icon="handshake">
    Ensures reliable communication by waiting for parent to signal readiness.
    ...
    **Signal Types Handled:**
    ...
    The implementation handles multiple signal types to work with different MCP-UI clients.
  </Accordion>

  <Accordion title="Iframe Lifecycle Management" icon="rotate">
    The `useIframeLifecycle` hook manages the complete lifecycle:
  ...
  <Accordion title="Dynamic Tool Updates" icon="arrows-rotate">
    The integration listens for `tools/list_changed` notifications, allowing embedded apps to add or remove tools dynamically.

    <Note>
    This is perfect for applications with conditional features, user permissions, or state-dependent capabilities.
    </Note>
  </Accordion>
</AccordionGroup>
...
<Tip>
**Zero build step!** You can build fully-functional MCP-UI apps with pure HTML/CSS/JavaScript. Just include `@mcp-b/global` via CDN and start coding.
</Tip>
...
<Info>
WebMCP provides two transport options for iframe communication. Choose based on your origin requirements and architecture needs.
</Info>

<Tabs>
  <Tab title="Iframe Transports" icon="window">
    **IframeParentTransport + IframeChildTransport**
    ...
    **Features:**
    - Cross-origin security with `targetOrigin` and `allowedOrigins`
    - Explicit lifecycle management
    - Production-ready (used in mcp-ui-webmcp repo)
  ...
  <Tab title="Tab Transports" icon="browsers">
    ...
    **Features:**
    ...
    - Simplified same-origin communication
    - Multiple server support

    ```typescript
    // Automatic discovery on same origin
...
<Tip>
**Production Recommendation:** The [mcp-ui-webmcp repository](https://github.com/WebMCP-org/mcp-ui-webmcp) uses IframeParentTransport and IframeChildTransport for production-ready cross-origin communication.
</Tip>
