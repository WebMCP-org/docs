...
<Note>
MCP-UI serves HTML/iframes that contain MCP servers. The parent page connects using IframeParentTransport or TabClientTransport and automatically discovers all tools inside the iframe.
</Note>

<Info>
See [Building MCP-UI Apps](/building-mcp-ui-apps) for a complete guide. This page focuses on integration patterns and architecture.
</Info>
...
The parent transport automatically discovers and connects to the MCP server inside the iframe.
...
Production implementation from the [mcp-ui-webmcp repository](https://github.com/WebMCP-org/mcp-ui-webmcp).
...
<AccordionGroup>
  <Accordion title="Shared Logic Pattern" icon="code-merge">
    UI handlers and WebMCP tools call the same underlying functions (`game.makeMove()`, `game.reset()`).
  </Accordion>
  ...
  <Accordion title="Parent-Ready Protocol" icon="handshake">
    Handles multiple ready signal types:
    - `ui-lifecycle-iframe-ready`
    - `parent-ready`
    - `ui-lifecycle-iframe-render-data`
    - `ui-message-response` / `ui-message-received`
  </Accordion>
  ...
  <Accordion title="Iframe Lifecycle Management" icon="rotate">
    ...
    6. Setup cleanup on unmount
  </Accordion>

  <Accordion title="Dynamic Tool Updates" icon="arrows-rotate">
    Listens for `tools/list_changed` notifications to add or remove tools dynamically.
  </Accordion>
</AccordionGroup>

## Vanilla JavaScript Example

Build MCP-UI apps with pure HTML/CSS/JavaScript. Include `@mcp-b/global` via CDN.
...
<CardGroup cols={2}>
  <Card title="Automatic Communication" icon="magic-wand-sparkles">
    IframeParentTransport handles communication
  </Card>
...
<Tabs>
  <Tab title="Iframe Transports" icon="window">
    **Use for different origins (most common)**
