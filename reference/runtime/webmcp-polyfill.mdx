...
<Note>
The polyfill does not provide `listTools()` or `callTool()` on `navigator.modelContext`. For tool listing and execution, use `navigator.modelContextTesting` (when the testing shim is enabled) or switch to [`@mcp-b/global`](/reference/runtime/global).
</Note>
...
For full inference documentation, see [@mcp-b/webmcp-types](/reference/runtime/webmcp-types).
