...
```javascript "Tool naming patterns" lines icon="square-js"
...
```javascript "Tool descriptions" lines icon="square-js" expandable
navigator.modelContext.registerTool({ // [!code --]
...
<Tabs>
  <Tab title="Good: Consolidated Tools">
    ```javascript "Consolidated tool example" lines icon="check"
  ...
  <Tab title="Avoid: Too Many Single-Purpose Tools">
    ```javascript "Multiple single-purpose tools" lines icon="x"
...
```typescript "Zod validation with descriptions" twoslash lines icon="check" expandable
import { z } from 'zod';
...
<Tabs>
  <Tab title="Good: Markdown">
    ```javascript "Markdown response format" lines icon="check" expandable
...
*Showing ${results.length} of ${results.total} total results*`;
  ...
  <Tab title="Avoid: JSON">
    ```javascript "JSON response format" lines icon="x"
...
```javascript "Secure data exposure" lines icon="shield"
async execute({ userId }) { // [!code --]
  const user = await getUser(userId); // [!code --]
 // [!code --]
  return { // [!code --]
    content: [{ // [!code --]
      type: "text", // [!code --]
      text: JSON.stringify(user) // ❌ Might include sensitive fields! // [!code --]
    }] // [!code --]
  }; // [!code --]
} // [!code --]
...
async execute({ userId }) { // [!code ++]
  const user = await getUser(userId); // [!code ++]
 // [!code ++]
  // ✅ Only include safe fields in response // [!code ++]
  return { // [!code ++]
    content: [{ // [!code ++]
      type: "text", // [!code ++]
...
**Member since:** ${new Date(user.createdAt).toLocaleDateString()}` // [!code ++]
    }] // [!code ++]
  }; // [!code ++]
  // ✅ NOT including: password hash, internal IDs, private notes, session tokens // [!code ++]
} // [!code ++]
```
...
<Tabs>
  <Tab title="Good: Optimistic Update">
    ```javascript "Optimistic update pattern" lines icon="bolt" expandable highlight={9-11,15-17}
...
**Total:** $${cartState.getTotal()}`;
  ...
  <Tab title="Avoid: Blocking on API">
    ```javascript "Blocking API call pattern" lines icon="x" highlight={6-10}
...
```typescript "React integration with useWebMCP" twoslash lines icon="react" expandable highlight={8-31}
...
<CardGroup cols={2}>
  <Card
    title="Tool Design Concepts"
    icon="hammer"
    href="/concepts/tool-design"
  >
    Deep dive into tool design philosophy and patterns
  </Card>
  ...
  >
    Security architecture for multi-agent environments
  </Card>
